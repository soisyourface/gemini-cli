...
----------------------------------------------------------------
  ...
  useEffect(() => {
    if (isTransitioningAltBuffer) {
      ...
      }, 150); // 150ms ensures Ink flushes the 'display: none' layout to the alternate buffer BEFORE we exit
    ...
    (key: Key): boolean => {
      ...
      if (keyMatchers[Command.TOGGLE_ALTERNATE_BUFFER](key)) {
        ...
        if (next) {
          ...
          // Exiting alternate buffer mode: Trigger a delay to let Ink clear its alternate buffer frame
  ...
  const uiState: UIState = useMemo(
    () => ({
      ...
      historyManager,
      isAlternateBuffer: isAlternateBuffer && !isTransitioningAltBuffer,
